...
export default function AgogeLibreActividades({ onBack }: { onBack: () => void }) {
  ...
  const handleChange = (idx: number, field: 'titulo' | 'descripcion', value: string) => {
    setRutinas(prev => prev.map((r, i) => i === idx ? { ...r, [field]: value } : r));
  };
      ...
      <div className="min-h-screen bg-white flex flex-col justify-between">
        <div className="pt-6 pb-4 px-4">
          <div className="flex items-center mb-6">
            ...
            <h1 className="text-2xl font-bold font-roboto text-center flex-grow">Agoge Libre</h1>
            <div className="w-10"></div>
          </div>
          <h2 className="text-2xl font-bold mb-6 text-center font-roboto">Rutinas personalizadas</h2>
          {rutinas.map((rutina, idx) => (
            <div key={idx} className="flex items-center bg-white rounded-2xl shadow-md mb-6 overflow-hidden">
              <div className="flex flex-col items-center justify-center w-1/5 min-w-[64px] py-4">
                <img src="/agogeLibre.png" alt="icono" className="w-14 h-14 object-contain" />
              </div>
